...
class Buku extends CI_Controller
{
    public function __construct()
    {
        ...
        cek_login();
        // $this->load->model('ModelUser'); // Load ModelUser jika belum diload
        // $this->load->model('ModelBuku'); // Load ModelBuku jika belum diload
        // $this->load->library('form_validation'); // Load form_validation jika belum diload
        // $this->load->library('upload'); // Load library upload jika belum diload
    }
    //manajemen Buku
    public function index()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('tambah', '<div 
class="alert alert-success alert-message" role="alert">Buku berhasil ditambah</div>');
            redirect('buku');
        }
    }
    ...
    public function kategori()
    {
        ...
        $data['user'] = $this->ModelUser->cekData(['email' => $this->session->userdata('email')])->row_array();
        $data['kategori'] = $this->ModelBuku->getKategori()->result_array();
        $this->form_validation->set_rules('nama_kategori', 'kategori', 'required|min_length[3]', [
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->session->set_flashdata('tambah', '<div 
class="alert alert-success alert-message" role="alert">Kategori berhasil ditambah</div>');
    ...
    public function hapusKategori()
    {
        ...
        $this->session->set_flashdata('hapus', '<div 
class="alert alert-danger alert-message" role="alert">Kategori berhasil dihapus</div>');
        redirect('buku/kategori');
    }
    ...
    public function ubahBuku()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($data['submit'])) {
                ...
                $this->session->set_flashdata('ubah', '<div class="alert alert-primary alert-message" role="alert">Profil Berhasil diubah </div>');
                redirect('buku');
            }
        }
    }
    ...
    public function hapusBuku()
    {
        ...
        $this->session->set_flashdata('hapus', '<div 
class="alert alert-danger alert-message" role="alert">Buku berhasil dihapus</div>');
        redirect('buku');
    }
}
